...
class CashBookReportXlsx(ReportXlsx):
    ...
    def generate_report_template(self):
        self.sheet.merge_range(
            'E1:J1', u'Mẫu số S07a-DN', self.format_template_title)

        msg = u'(Ban hành theo Thông tư số 200/2014/TT-BTC, Ngày 22/12/2014 của BTC)'
        self.sheet.merge_range(
            'E2:J2', msg, self.format_center)

    def generate_report_title(self):
        self.sheet.merge_range(
            'A4:J4', u'SỔ QUỸ TIỀN MẶT', self.format_report_title)

        self.sheet.merge_range(
            'A5:J5', u'Loại quỹ: %s' % self.get_account_info(),
            self.format_center)
    ...
    def generate_content_header(self):
        ...
        self.sheet.merge_range(
            'B8:B9', u'Ngày, tháng chứng từ', self.format_table_bold_center)

        self.sheet.merge_range(
            'C8:D8', u'Số hiệu chứng từ', self.format_table_bold_center)

        self.sheet.write(
            'C9', u'Thu', self.format_table_bold_center)

        self.sheet.write(
            'D9', u'Chi', self.format_table_bold_center)

        self.sheet.merge_range(
            'E8:E9', u'Diễn giải', self.format_table_bold_center)

        self.sheet.merge_range(
            'F8:F9', u'Tài khoản đối ứng', self.format_table_bold_center)

        self.sheet.merge_range(
            'G8:I8', u'Số phát sinh', self.format_table_bold_center)

        self.sheet.write(
            'G9', u'Nợ', self.format_table_bold_center)

        self.sheet.write(
            'H9', u'Có', self.format_table_bold_center)

        self.sheet.write(
            'I9', u'Tồn', self.format_table_bold_center)

        self.sheet.merge_range(
            'J8:J9', u'Ghi chú', self.format_table_bold_center)
    ...
    def generate_main_content(self):
        ...
        for line in main_contents:
            ...
            self.sheet.write(
                'C%s' % self.row_position, line['ref_debit'],
                self.format_table_left)

            self.sheet.write(
                'D%s' % self.row_position, line['ref_credit'],
                self.format_table_left)

            self.sheet.write(
                'E%s' % self.row_position, line['narration'] or '',
                self.format_table)

            self.sheet.write(
                'F%s' % self.row_position, line['counterpart_account'],
                self.format_table_right)

            self.sheet.write(
                'G%s' % self.row_position, line['debit'] or '',
                self.format_table)

            self.sheet.write(
                'H%s' % self.row_position, line['credit'] or '',
                self.format_table)

            self.sheet.write(
                'I%s' % self.row_position, line['remain'] or '',
                self.format_table)

            self.sheet.write(
                'J%s' % self.row_position, line['description'],
                self.format_table_left)
            self.row_position += 1
        ...
        self.sheet.write(
            'E%s' % self.row_position, u'Cộng số phát sinh',
        ...
        self.sheet.set_row(self.row_position - 1, 20)
        self.sheet.write(
            'E%s' % self.row_position, u'Số dư cuối kỳ',
            self.format_table_bold_left)

        self.sheet.write(
            'I%s' % self.row_position, remain_balance or '',
            self.format_table_bold_right)
        return True
    ...
    def generate_footer_content(self):
        ...
        self.sheet.merge_range(
            'G{0}:J{0}'.format(self.row_position),
            u'Ngày ... tháng ... năm ...', self.format_center)
        self.row_position += 1

        self.sheet.merge_range(
            'A{0}:C{0}'.format(self.row_position),
    ...
    def write_begining_balance(self):
        ...
        self.sheet.set_row(self.row_position - 1, 20)
        self.sheet.write(
            'E%s' % self.row_position, u'Số dư đầu kỳ',
            self.format_table_bold_left)
        self.sheet.write(
            'I%s' % self.row_position, balance or 0.0,
            self.format_table_bold_right)
        self.row_position += 1
    ...
    def prepare_main_content(self):
        ...
        SQL = """
            SELECT
                ...
                CASE WHEN  acc.id is null then acc_dr.code else acc_cr.code
                END as counterpart_account,
...
CashBookReportXlsx('report.cash_book_report_xlsx', 'cash.book.wizard')
